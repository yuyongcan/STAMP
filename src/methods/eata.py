...
class EATA(nn.Module):
    ...
    def forward(self, x):
        ...
        #     self.os_queue.extend(entropy.cpu().numpy())
        #     self.os_queue = self.os_queue[-1000:]
        #     os = entropy - torch.tensor(np.mean(self.os_queue)).cuda()

        # max_logit_src, src_idx = torch.max(self.model_src(x), dim=1, keepdim=True)
        # max_logit_norm, norm_idx = torch.max(outputs, dim=1, keepdim=True)
